...
{
	//******************************** added ******************************
	private static List<net.minecraft.network.packet.IPacketHandler> packetHandlers;
	private boolean callPacketOut(Packet p) {
		for (net.minecraft.network.packet.IPacketHandler handler : getPacketHandlers()) {
	...
	public static java.util.List<net.minecraft.network.packet.IPacketHandler> getPacketHandlers() {
		if (packetHandlers == null) {
			synchronized (net.minecraft.network.packet.IPacketHandler.class) {
				if (packetHandlers == null) {
					packetHandlers = new ArrayList<net.minecraft.network.packet.IPacketHandler>();
				}
			}
		}
    ...
    {
        if (!this.isServerTerminating && callPacketOut(par1Packet)) // added '' && callPacketOut(par1Packet)''
        {
